...
class ModelInstallInstall extends Model {
	public function database($data) {
		...
		$db->query("INSERT INTO `" . $data['db_prefix'] . "user` SET user_id = '1', user_group_id = '1', username = '" . $db->escape($data['username']) . "', salt = '', password = '" . $db->escape(password_hash($data['password'], PASSWORD_DEFAULT)) . "', fullname = 'John', email = '" . $db->escape($data['email']) . "', status = '1', date_added = NOW()");
